...
class MaskedResidualBlock(nn.Module):
    ...
    def forward(self, inputs, context=None):
        ...
        if context is not None:
            temps = F.glu(torch.cat((temps, self.context_layer(context)), dim=-2), dim=-2)
        return inputs + temps
...
class MADE(nn.Module):
    ...
    Appear to work over batched inputs
    Context is not used
    """
    ...
    def __init__(
        ...
        # Initial layer. Note needs different input degree due to preprocessing
    ...
    def forward(self, inputs, context=None):
        ...
        if context is not None:
            outputs += self.context_layer(context)  # add context to outputs
